...
### 현금 선결제 플로우
선결제 손님 테이블에서 현금 결제를 눌렀을 때 해당 주문은 승인 대기 주문으로 변경되고 아래의 플로우에 따른다
1. 승인 플로우
- 선결제 테이블에서 메뉴를 담은 뒤 현금 결제 버튼 클릭
- 팝업창 → 곧 직원이 결제하러 올 예정이에요! 결제 후 주문을 도와드릴게요
    1. 소리가 나야 함
    2. 아직 결제 되지 않았고, 주문이 들어가지 않은 걸 알려야 함
    3. 확인 버튼을 누르기 전까지 안꺼지게끔 개발
- 직원) 홀 관리에서 승인 대기중인 주문 확인
- 직원) 손님 테이블로 이동 후 현금 결제
- 직원) pos로 가서 해당 테이블 선택 후 승인 버튼
- 승인 & 거부 버튼 자리에 현금결제 & 카드결제 버튼 생김 
- 결제 후 팝업창 
    1. pos 테이블 목록 페이지) 결제가 완료되었어요! 메뉴를 준비할게요.
    2. 손님테이블) 결제가 완료되었어요! 메뉴를 준비할게요.
        1. **소리 나서 손님이 알 수 있게끔 설정**
2. 거부 플로우
- 선결제 테이블에서 메뉴를 담은 뒤 현금 결제 버튼 클릭
- 팝업창 → 곧 직원이 결제하러 올 예정이에요! 결제 후 주문을 도와드릴게요
    1. **소리가 나야 함**
    2. 아직 결제 되지 않았고, 주문이 들어가지 않은 걸 알려야 함
- 직원) 홀 관리에서 승인 대기중인 주문 확인
- 직원) 손님 테이블로 이동 후 주문 거부 사유 안내
- 직원) pos에서 해당 테이블 선택 후 거부 버튼
- 거부 후 팝업창
  - pos)
    1. 경고팝업 - 테이블의 주문을 거부하시겠습니까?
    2. 테이블 주문이 거부되었습니다. 해당 테이블로 돌아가 손님께 재주문을 요청해주세요
  - 손님테이블) 주문이 거부되었어요. 직원의 안내에 따라주세요!
    - **소리 필수**
    - **확인 버튼을 누르기 전까지 안꺼지게끔 개발**
- 손님은 재주문
- 다시 들어온 재주문 확인 후 승인 버튼
- 승인 & 거부 버튼 자리에 현금결제 & 카드결제 버튼 생김
- 결제 후 팝업창
    1. pos 테이블 목록 페이지) 결제가 완료되었어요! 메뉴를 준비할게요.
    2. 손님테이블) 결제가 완료되었어요! 메뉴를 준비할게요.
